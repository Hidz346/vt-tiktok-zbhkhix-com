...
// ===== LOCAL ANNOUNCEMENT (MANUAL) =====
const LOCAL_ANNOUNCEMENT = {
    text: "🔥 Selamat datang di HidzXrat",
    expireAt: new Date("2027-01-01").getTime()
};

// Admin & owner accounts
const ADMIN_ACCOUNTS = ['zaaa', 'onedev', 'Hidz'];
...
// ===== INBOX REPLY FUNCTIONS (DIPERBAIKI TAMPILANNYA) =====
function listenToReplies() {
    ...
    const userRepliesRef = db.ref(`user_replies/${currentUser}`);
    repliesListener = userRepliesRef.on('value', (snap) => {
        if (!snap.exists()) {
...
function openInboxReply() {
    if (!currentUser) return;
    const fullscreen = document.getElementById('inboxFullscreen');
    const content = document.getElementById('inboxContent');
    fullscreen.style.display = 'flex';
    content.innerHTML = '<div class="inbox-empty"><i class="fas fa-spinner fa-pulse"></i> Memuat...</div>';
    
    db.ref(`user_replies/${currentUser}`).once('value', (snap) => {
        const replies = snap.val() || {};
        const sorted = Object.entries(replies).sort((a, b) => b[1].timestamp - a[1].timestamp);
        if (sorted.length === 0) {
            content.innerHTML = '<div class="inbox-empty"><i class="fas fa-inbox"></i> Belum ada balasan dari admin</div>';
            return;
        }
        content.innerHTML = '';
        
        for (const [id, reply] of sorted) {
            const reportId = reply.reportId;
            
            const card = document.createElement('div');
            card.className = `inbox-card ${!reply.read ? 'unread' : ''}`;
            
            // Admin Section
            const adminDiv = document.createElement('div');
            adminDiv.className = 'inbox-admin-section';
            adminDiv.innerHTML = `
                <div class="inbox-admin-icon"><i class="fas fa-user-shield"></i></div>
                <div class="inbox-admin-name">${escapeHtml(reply.adminName)}</div>
                <div class="inbox-date">${formatDateSafely(reply.timestamp)}</div>
            `;
            card.appendChild(adminDiv);
            
            // Report Section
            const reportDiv = document.createElement('div');
            reportDiv.className = 'inbox-report-section';
            reportDiv.innerHTML = `
                <div class="inbox-report-label"><i class="fas fa-flag"></i> Laporan yang Anda kirimkan:</div>
                <div class="inbox-report-message">Memuat laporan...</div>
            `;
            card.appendChild(reportDiv);
            
            if (reportId) {
                db.ref(`reports/${reportId}/message`).once('value', (msgSnap) => {
                    const userMessage = msgSnap.val() || 'Laporan tidak ditemukan';
                    const msgDiv = reportDiv.querySelector('.inbox-report-message');
                    if (msgDiv) msgDiv.innerHTML = escapeHtml(userMessage);
                });
            } else {
                const msgDiv = reportDiv.querySelector('.inbox-report-message');
                if (msgDiv) msgDiv.innerHTML = 'Laporan tidak ditemukan';
            }
            
            // Answer Section
            const answerDiv = document.createElement('div');
            answerDiv.className = 'inbox-answer-section';
            answerDiv.innerHTML = `
                <div class="inbox-answer-label"><i class="fas fa-reply"></i> Balasan dari Admin:</div>
                <div class="inbox-answer-message">${escapeHtml(reply.message)}</div>
            `;
            card.appendChild(answerDiv);
            
            // Note Section
            const noteDiv = document.createElement('div');
            noteDiv.className = 'inbox-note-section';
            const noteText = reply.note && reply.note.trim() !== '' ? escapeHtml(reply.note) : '-';
            noteDiv.innerHTML = `
                <div class="inbox-note-label"><i class="fas fa-sticky-note"></i> Catatan Admin:</div>
                <div class="inbox-note-message">${noteText}</div>
            `;
            ...
            if (!reply.read) {
                db.ref(`user_replies/${currentUser}/${id}/read`).set(true);
            }
        }
...
function closeInboxFullscreen() {
    document.getElementById('inboxFullscreen').style.display = 'none';
}
...
function listenToAnnouncements() {
    ...
    db.ref('announcements').on('value', (snapshot) => {
        ...
        if (activeAnnouncement) {
            ...
            const viewersRef = db.ref(`announcements/${activeId}/viewers/${currentUser}`);
...
function loadLocalAnnouncement() {
    const btn = document.getElementById('floatingAnnouncementBtn');
    const badge = document.getElementById('announcementBadge');
    ...
    if (LOCAL_ANNOUNCEMENT.expireAt > now) {
        currentAnnouncementData = LOCAL_ANNOUNCEMENT;
    ...
    db.ref('registered_users/' + u).once('value').then(s => {
        ...
        if(d && d.password === p && new Date().getTime() < d.expiry) {
            ...
            const goToDashboard = function() {
                ...
                loadLocalAnnouncement();
                listenToReplies();
                
                checkVPNOnLoad();
                
                showNotification('Welcome HidzXrat');
            };
...
function logout() {
    ...
    localStorage.removeItem('phising_history');
    ...
    const fullscreens = document.querySelectorAll('.fullscreen-panel, .announcement-fullscreen, .inbox-fullscreen, .report-fullscreen, .chat-fullscreen, .photo-viewer, .downloader-page, .modal-overlay');
    fullscreens.forEach(fs => {
        if(fs && fs.style) fs.style.display = 'none';
    });
